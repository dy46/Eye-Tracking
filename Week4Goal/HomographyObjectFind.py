...
# def getFrame(queue, startFrame, endFrame, fourcc, fps, capSize, frcount):
...
#     # list of framecounts
#     # initialize as function of start frame
#     # pass the repective one in 
#     qList = []
...
#         beginFrame = i[0]
#         framecount = 0.0
#         for x in range(beginFrame):
#             framecount = framecount + (1.0/fps)*1000.0
#         frcount.append(framecount)

#     print bunches
#     getFrames = []
#     for i in range(processCount):
#         getFrames.append(mp.Process(target=getFrame, args=(qList[i], bunches[i][0], bunches[i][1], fourcc, fps, capSize, frcount[i])))
...
def edit(f, currentFrame):
    ...
    for i,img1 in enumerate(img):
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            ignore = True
            break
        # print str(mp.current_process()) + "framecount: " + str(framecount)

        if First == True:
            img2, x, y, idx, tail ,ignore= datamani.drawCircle(img2, framecount, videoData, idx, tail, fps)
            framecount = framecount + (1.0/fps)*1000.0
        img3, pos = drMatches.drawMatches(img1,kp1,img2,kp2,good, pos)
        First = False
        s[i][0]=dst[0][0][0];
